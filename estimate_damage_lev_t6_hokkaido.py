...
#    calc_nameori = 'flood_out4_' + calc
    ...
    cama_file1 = os.path.join(flood_path, 'flood_lev_c000_zero_hokkaidol4.bin')

#    cama_fileori = os.path.join(flood_path, calc_nameori+'.bin')
#    cama_fileori1 = os.path.join(flood_path, 'flood_out4_lev_c000_zero_hokkaidol4.bin')
...
#    df_home[calc] = df_cama['home_damage'].dropna().astype('int16').copy()
#    df_house[calc] = df_cama['house_damage'].dropna().astype('int16').copy()

    df_cama[df_cama['fld_dph'] > 0.1].to_csv('test19_'+calc_name+'.csv')
#    df_cama[df_cama['fld_dphori'] > 0.1].to_csv('test18_'+calc_name+'.csv')
    #df_cama[df_cama['home_damage'] > 2].to_csv('test12_'+calc_name+'.csv')
#    df_cama.drop(['fld_dph1', 'home_rate', 'floor'], axis=1, inplace=True)
#    df_cama.to_csv('test14_'+calc_name+'.csv')

out_csv = os.path.join(out_dir, 'total_hokkaido4e7')
