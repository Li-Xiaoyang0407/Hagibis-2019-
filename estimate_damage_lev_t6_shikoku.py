...
#df_cama = df_cama[['necessary_column_1', 'necessary_column_2']]
...
#    calc_nameori = 'flood_out4_' + calc
    ...
    cama_file1 = os.path.join(flood_path, 'flood_lev_c000_zero_shikokul4.bin')
    
#    cama_fileori = os.path.join(flood_path, calc_nameori+'.bin')
#    cama_fileori1 = os.path.join(flood_path, 'flood_out4_lev_c000_zero_shikokul4.bin')
...
#    df_cama['household'] = df_cama['household'].fillna(0).astype('int8')
#    df_cama['area'] = df_cama['area'].fillna(0).astype('int8')
#    df_cama['fld_dph1'] = df_cama['fld_dph1'].fillna(0).astype('int8')
#    df_cama['fld_dph'] = df_cama['fld_dph'].fillna(0).astype('int8')
#    df_cama['home_rate'] = df_cama['home_rate'].fillna(0).astype('int8')
#    df_cama['floor'] = df_cama['floor'].astype('float16')
#    df_cama['home_damage'] = df_cama['home_damage'].fillna(0).astype('int16')
#    df_cama['house_damage'] = df_cama['house_damage'].fillna(0).astype('int16')
#    df_home[calc] = df_cama['home_damage'].copy()
#    df_house[calc] = df_cama['house_damage'].copy() 

    df_home[calc] = df_cama['home_damage'].fillna(0).astype('float32').copy()
    df_house[calc] = df_cama['house_damage'].fillna(0).astype('float32').copy()
#    df_homeori[calc] = df_cama['home_damageori'].fillna(0).astype('float32').copy()
#    df_houseori[calc] = df_cama['house_damageori'].fillna(0).astype('float32').copy()
...
#    df_cama.to_csv('test14a_'+calc_name+'.csv')

out_csv = os.path.join(out_dir, 'total_shikoku4e9')
